...
class Polygons_scene():
  ...
  def set_up_animation(self):
    self.draw_scene()
    # start = (self.gui_robot.x(), self.gui_robot.y(), self.gui_robot._angle)
    # anim = gui.general_segment_animation(self.gui_robot, *start, self.gui_robot.x() - 500, self.gui_robot.y() - 300, 1, False)
    # gui.queue_animation(anim)
    # anim = gui.general_segment_animation(self.gui_robot, self.gui_robot.x() - 500, self.gui_robot.y() - 300, 1 , *start, True)
    # gui.queue_animation(anim)
    if len(self.path) == 0:
      return
    if len(self.path) == 1:
